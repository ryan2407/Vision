<?php namespace VisionMedia\LastPlayedWidget;

class LastPlayedWidget extends \WP_Widget {

	function __construct()
	{
		parent::__construct(
			'lastplayed_widget', // Base ID
			__( 'Vision Apps: Last Played', 'lastplayed_widget' ), // Name
			array( 'description' => __( 'This widget displays what was last played', 'lastplayed_widget' ), ) // Args
		);
		add_action('wp_enqueue_scripts', array($this, 'loadScripts'));
		add_action( 'wp_ajax_nopriv_lastplayed', array($this, 'LastPlayed'));
		add_action( 'wp_ajax_lastplayed', array($this, 'LastPlayed'));
		add_shortcode( 'lastplayed', array($this, 'ShortCode') );
	}
	...
	public function LastPlayed()
	{
		...
		$url = 'http://dev.vision.org.au/radio-api/json/last_played.php?tz='.$tz.'&lh='.$lh.$dc.$rs.$re;
		//  Initiate curl
		$ch = curl_init();
